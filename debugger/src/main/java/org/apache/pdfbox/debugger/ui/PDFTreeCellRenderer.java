...
public class PDFTreeCellRenderer extends DefaultTreeCellRenderer {
  ...
  public Component getTreeCellRendererComponent(JTree tree, Object nodeValue, boolean isSelected, boolean expanded, boolean leaf, int row, boolean componentHasFocus) {
    Component component = super.getTreeCellRendererComponent(tree, toTreeObject(nodeValue), isSelected, expanded, leaf, row, componentHasFocus);
    ...
    if (nodeValue instanceof MapEntry || nodeValue instanceof ArrayEntry) {
      ...
      if (object instanceof String && ((String) object).length() > 0) {
        ...
        if (item instanceof COSObject) {
          ...
          stringResult += " [" + indirect.getObjectNumber() + " " + indirect.getGenerationNumber() + " R]";
        }
  ...
  private String toTreePostfix(Object nodeValue) {
    if (nodeValue instanceof COSDictionary) {
      ...
      if (COSName.ANNOT.equals(dict.getCOSName(COSName.TYPE)) && COSName.WIDGET.equals(dict.getCOSName(COSName.SUBTYPE)) || dict.containsKey(COSName.T) && dict.containsKey(COSName.KIDS)) {
