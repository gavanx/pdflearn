  ...
  protected COSDictionary parseXref(long startXRefOffset) throws IOException {
    ...
    while (prev > 0 && prev != lastPrev) {
      ...
      if (source.peek() == X) {
        ...
        parseXrefTable(prev);
        if (!parseTrailer()) {
          throw new IOException("Expected trailer object at position: " + source.getPosition());
        }
          ...
          if (!parsedObjects.contains(objId)) {
            ...
            if (fileOffset != null && fileOffset != 0) {
              if (fileOffset > 0) {
                ...
                fileOffset = xrefTrailerResolver.getXrefTable().get(new COSObjectKey((int) -fileOffset, 0));
                if ((fileOffset == null) || (fileOffset <= 0)) {
  ...
  private void readValidStream(OutputStream out, COSNumber streamLengthObj) throws IOException {
    ...
    while (remainBytes > 0) {
      ...
      if (readBytes <= 0) {
        ...
        throw new IOException("read error at offset " + source.getPosition() + ": expected " + chunk + " bytes, but read() returns " + readBytes);
      }
  ...
  private void checkXrefOffsets() throws IOException {
    ...
    if (xrefOffset != null) {
      ...
      for (Entry<COSObjectKey, Long> objectEntry : xrefOffset.entrySet()) {
        ...
        if (objectOffset != null && objectOffset >= 0 && !checkObjectKeys(objectKey, objectOffset)) {
            ...
            for (COSObjectKey key : objStreams) {
              if (bfSearchCOSObjectKeyOffsets.containsKey(key)) {
                ...
                for (Long objNr : objects) {
                  ...
                  Long streamObjectOffset = bfSearchCOSObjectKeyOffsets.get(streamObjectKey);
      ...
      for (Entry<COSObjectKey, Long> entry : bfSearchCOSObjectKeyOffsets.entrySet()) {
        ...
        try {
          ...
          if (dictionary != null) {
            ...
            // info dictionary
            else if (dictionary.containsKey(COSName.MOD_DATE) && (dictionary.containsKey(COSName.TITLE) || dictionary.containsKey(COSName.AUTHOR) || dictionary.containsKey(COSName.SUBJECT) || dictionary.containsKey(COSName.KEYWORDS) || dictionary.containsKey(COSName.CREATOR) || dictionary.containsKey(COSName.PRODUCER) || dictionary.containsKey(COSName.CREATION_DATE))) {
