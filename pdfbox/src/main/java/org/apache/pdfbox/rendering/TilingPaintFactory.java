...
class TilingPaintFactory {
  ...
  private final Map<TilingPaintParameter, TilingPaint> weakCache = new WeakHashMap<TilingPaintParameter, TilingPaint>();

  TilingPaintFactory(PageDrawer drawer) {
    this.drawer = drawer;
  }

  TilingPaint create(PDTilingPattern pattern, PDColorSpace colorSpace, PDColor color, AffineTransform xform) throws IOException {
    TilingPaint paint;
