...
public class PageDrawer extends PDFGraphicsStreamEngine {
  ...
  private void adjustRectangle(Rectangle2D r) {
    ...
    if (pageRotation == 90) {
      r.setRect(pageSize.getHeight() * m.getScalingFactorY() - r.getY() - r.getHeight(), r.getX(), r.getWidth(), r.getHeight());
    }
    if (pageRotation == 180) {
      r.setRect(pageSize.getWidth() * m.getScalingFactorX() - r.getX() - r.getWidth(), pageSize.getHeight() * m.getScalingFactorY() - r.getY() - r.getHeight(), r.getWidth(), r.getHeight());
    }
    if (pageRotation == 270) {
      r.setRect(r.getY(), pageSize.getWidth() * m.getScalingFactorX() - r.getX() - r.getWidth(), r.getWidth(), r.getHeight());
    }
  }
  ...
  private void drawAnnotationLinkBorder(PDAnnotationLink link) throws IOException {
    ...
    graphics.setClip(null);
    if (ab.underline) {
      graphics.drawLine((int) rectangle.getLowerLeftX(), (int) rectangle.getLowerLeftY(), (int) (rectangle.getLowerLeftX() + rectangle.getWidth()), (int) rectangle.getLowerLeftY());
    } else {
      graphics.drawRect((int) rectangle.getLowerLeftX(), (int) rectangle.getLowerLeftY(), (int) rectangle.getWidth(), (int) rectangle.getHeight());
    }
  ...
  private final class TransparencyGroup {
    ...
    private TransparencyGroup(PDTransparencyGroup form, boolean isSoftMask, Matrix ctm, PDColor backdropColor) throws IOException {
      ...
      pageSize = new PDRectangle(minX / Math.abs(m.getScalingFactorX()), minY / Math.abs(m.getScalingFactorY()), (float) bounds.getWidth() / Math.abs(m.getScalingFactorX()), (float) bounds.getHeight() / Math.abs(m.getScalingFactorY()));
    ...
    private BufferedImage create2ByteGrayAlphaImage(int width, int height) {
       ...
       */
      final ColorModel CM_GRAY_ALPHA = new ComponentColorModel(ColorSpace.getInstance(ColorSpace.CS_GRAY), true, false, Transparency.TRANSLUCENT, DataBuffer.TYPE_BYTE);
    ...
    public Rectangle2D getBounds() {
      ...
      // Flip y
      return new Rectangle2D.Double(minX - pageSize.getLowerLeftX() * m.getScalingFactorX(), size.getY() - minY - height + pageSize.getLowerLeftY() * m.getScalingFactorY(), width, height);
    }
  }
}
